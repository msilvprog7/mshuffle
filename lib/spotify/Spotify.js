...
var SpotifyAPI = (function () {
		...
		return {
			...
			getPlaylists: function (req, onSuccess, onError, url, playlists) {
				...
				url = (url) ? url : 'https://api.spotify.com/v1/me/playlists';
		        	...
		        	if (error) {
		        		...
		        		playlists.push.apply(playlists, body.items.map(function (playlist) {
		        			...
		        			return {
		        				...
		        				image: (playlist.images && playlist.images.length > 0) ? playlist.images[0] : null,
			...
			getPlaylist: function (req, ownerId, playlistId, onSuccess, onError, url, playlist) {
				// Use defaults or defined values
				url = (url) ? url : `https://api.spotify.com/v1/users/${ownerId}/playlists/${playlistId}`;
				playlist = (playlist) ? playlist : { id: undefined, name: undefined, description: undefined, image: undefined, owner: undefined, songs: [] };
		        ...
		        request.get(options, function(error, response, body) {
		        	if (error) {
		        		...
		        		// Set details once
		        		playlist.id = (playlist.id) ? playlist.id : body.id;
		        		playlist.name = (playlist.name) ? playlist.name : body.name;
		        		playlist.description = (playlist.description) ? playlist.description : body.description;
		        		playlist.image = (!playlist.image && body.images && 
		        			body.images.length > 0) ? body.images[0] : playlist.image;
		        		playlist.owner = (playlist.owner) ? playlist.owner : body.owner;
						
		        		// Extend playlist tracks (note that if recursively paging, the tracks will be the whole body response)
		        		var tracks = (body.tracks) ? body.tracks : body;
			...
			/**
			 * Get a list of related artist ids
			 */
			getSimilarArtists: function (access_token, artistId, onSuccess, onError) {
				// Use defaults or defined values
		        ...
		        request.get(options, function(error, response, body) {
		        	if (error && onError) {
		        		// Send error
		        		onError(error.status, error.message);
		        	} else if (body.error && onError) {
		        		// Send error
		        		onError(body.error.status, body.error.message);
		        	} else if (body) {
		        		// Send success with list of similar artist ids
						var similarArtists = (body.artists && body.artists.length > 0) ? body.artists : [];
						onSuccess(similarArtists.map(function (artist) {
							// console.log(`Similar artist: ${artist.id}`);
							return artist.id;
						}));
					}
				});
